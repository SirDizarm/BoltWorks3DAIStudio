...
const geometryBounds = object => {
  ...
  return { halfX: (Math.max(...xs) - Math.min(...xs)) * .5, halfZ: (Math.max(...zs) - Math.min(...zs)) * .5 };
};
...
(async () => {
  ...
  try {
    ...
    for (const asset of selectedAssets) {
      ...
      const centerX = project.scene.objects.reduce((sum, object) => sum + Number(object.position?.[0] || 0), 0) / project.scene.objects.length;
      assert(Math.abs(centerX) < .35, `${asset.slug} should remain centered, got mean X ${centerX}`);
