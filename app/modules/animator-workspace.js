...
function initializeAnimatorWorkspace(){
 ...
 if(io){const files=document.createElement('div');files.className='bws-model-file-actions';files.setAttribute('role','group');files.setAttribute('aria-label','Model import and export');for(const id of ['importGlbBtn','importGlbFile','exportGlbBtn','importGltfBtn','importGltfFile','exportGltfBtn']){const element=document.getElementById(id);if(element)files.append(element);}document.querySelector('#toolbarProjectFilesGroup')?.append(files);}
