...
function renderPluginManager() {
  ...
  els.pluginList.innerHTML = plugins.map(plugin => `<div class="plugin-row" data-plugin-row="${bwsEscapeHtml(plugin.id)}"><div class="plugin-summary"><strong>${bwsEscapeHtml(plugin.name)}</strong><span>v${bwsEscapeHtml(plugin.version)} · ${bwsEscapeHtml(plugin.id)} · ${plugin.bundled ? "Included compatibility module" : "Installed package"}</span></div><div class="plugin-actions"><span class="plugin-ready">${plugin.enabled ? "On" : "Off"}</span><button type="button" data-plugin-toggle="${bwsEscapeHtml(plugin.id)}">${plugin.enabled ? "Disable" : "Enable"}</button>${!plugin.bundled && plugin.enabled && plugin.runtime === "sandbox-html" ? `<button type="button" data-plugin-open="${bwsEscapeHtml(plugin.id)}">Open</button>` : ""}${plugin.bundled ? "" : `<button type="button" class="danger" data-plugin-remove="${bwsEscapeHtml(plugin.id)}">Remove</button>`}</div></div>`).join("");
}
