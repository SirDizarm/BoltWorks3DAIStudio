// Vendored from Three.js 148ef33ecb6d2502ff796d4554abd1549c95d519. MIT license: ../three-LICENSE.txt
...
class USDCParser {
	...
	_readPaths() {
		...
		if ( this.version.major === 0 && this.version.minor < 4 ) {
			...
			const encodedPaths = reader.readUint64();
            if(!Number.isSafeInteger(encodedPaths)||encodedPaths<0||encodedPaths>numPaths)throw new Error('Invalid USD encoded path count');
	...
	_readReferenceList() {
        const r=this.reader, flags=r.readUint8(), lists={};
        for(const flag of [2,4,32,64,8,16])if(flags&flag){
            const count=r.readUint64();
            if(!Number.isSafeInteger(count)||count<0||count>(this.buffer.byteLength-r.tell())/32)throw new Error('Invalid USD reference count');
            lists[flag]=[];
            for(let i=0;i<count;i++){
                const assetPath=this.tokens[this.strings[r.readUint32()]];
                const primPath=this.paths[r.readUint32()]||'';
                const offset=r.readFloat64(),scale=r.readFloat64();
                // Reference custom data is not scene composition data. Skip its recursive values.
                const entries=r.readUint64();
                if(!Number.isSafeInteger(entries)||entries<0||entries>100000)throw new Error('Invalid USD reference metadata');
                for(let j=0;j<entries;j++){
                    r.readUint32();const start=r.tell(),relative=r.readInt64();
                    const next=start+relative+8;
                    if(next<0||next>this.buffer.byteLength)throw new Error('Invalid USD reference metadata offset');
                    r.seek(next);
                }
                if(typeof assetPath!=='string')throw new Error('Invalid USD reference asset index');
                lists[flag].push({assetPath,primPath,offset,scale});
            }
        }
        const identity=ref=>ref.assetPath+'|'+ref.primPath;
        let result=flags&1?(lists[2]||[]):[...(lists[32]||[]),...(lists[4]||[]),...(lists[64]||[])];
        const deleted=new Set((lists[8]||[]).map(identity));
        result=result.filter((ref,i,all)=>!deleted.has(identity(ref))&&all.findIndex(other=>identity(other)===identity(ref))===i);
        if(lists[16]){const ordered=lists[16].map(identity);result.sort((a,b)=>{const ai=ordered.indexOf(identity(a)),bi=ordered.indexOf(identity(b));return (ai<0?ordered.length:ai)-(bi<0?ordered.length:bi);});}
        return result;
    }
	...
	_readScalarValue( type ) {
		...
		switch ( type ) {
			...
			case TypeEnum.ReferenceListOp:
                return this._readReferenceList();
            case TypeEnum.StringVector: {
                const count=reader.readUint64();
                if(!Number.isSafeInteger(count)||count<0||count>(this.buffer.byteLength-reader.tell())/4)throw new Error('Invalid USD string vector');
                return Array.from({length:count},()=>this.tokens[this.strings[reader.readUint32()]]);
            }
            case TypeEnum.LayerOffsetVector: {
                const count=reader.readUint64();
                if(!Number.isSafeInteger(count)||count<0||count>(this.buffer.byteLength-reader.tell())/16)throw new Error('Invalid USD layer offsets');
                return Array.from({length:count},()=>[reader.readFloat64(),reader.readFloat64()]);
            }
