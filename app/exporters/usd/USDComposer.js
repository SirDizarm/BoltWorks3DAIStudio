...
class USDComposer {
	...
	compose( parsedData, assets = {}, variantSelections = {}, basePath = '' ) {
		...
		const metersPerUnit = rootFields.metersPerUnit ?? 0.01;
	...
	_resolveReference( refValue, localVariants = {} ) {
		...
		if ( ! referencedData ) throw new Error( 'Missing USD layer: ' + resolvedPath + '. Select all companion layers or import a complete package.' );
		...
		if ( referencedData.specsByPath ) {
			...
			this.texturePromises.push( ...composer.texturePromises );
	...
	_findUVPrimvar( fields ) {
		...
		const uvs = fields[ 'primvars:st' ] || fields[ 'primvars:st0' ] || fields[ 'primvars:UVMap' ];
		const uvIndices = fields[ 'primvars:st:indices' ] || fields[ 'primvars:st0:indices' ];
	...
	_applyMdlPreview( material, fields ) {
        this.materialWarnings ||= new Set();
        this.materialWarnings.add('NVIDIA MDL materials approximated using standard color, normal, roughness, metallic, opacity and occlusion maps. Advanced MDL effects are not preserved.');
        const maps={diffuse_texture:'map',normalmap_texture:'normalMap',reflectionroughness_texture:'roughnessMap',metallic_texture:'metalnessMap',ao_texture:'aoMap',opacity_texture:'alphaMap'};
        for(const [input,property]of Object.entries(maps)){
            const path=fields['inputs:'+input];if(!path)continue;
            const texture=this._loadTextureFromPath(path);
            texture.colorSpace=property==='map'?SRGBColorSpace:NoColorSpace;
            material[property]=texture;
        }
        material.metalness=material.metalnessMap?1:0;
        material.roughness=typeof fields['inputs:reflection_roughness']==='number'?fields['inputs:reflection_roughness']:1;
        const opacity=fields['inputs:opacity_multiplier'];if(typeof opacity==='number')material.opacity=Math.max(0,Math.min(1,opacity));
        if(material.alphaMap){material.alphaTest=0.35;material.side=2;}
        if(material.opacity<1)material.transparent=true;
        material.userData.usdMdlApproximation=true;
    }
	...
	_applyMaterial( material, materialPath ) {
		...
		for ( const path of shaderPaths ) {
			...
			} else if ( shaderAttrs[ 'info:mdl:sourceAsset' ] ) {
                this._applyMdlPreview( material, shaderAttrs );
	...
	_loadTexture( filePath, textureAttrs, transformAttrs ) {
		const cleanPath = filePath.replace( /^@|@$/g, '' ).replaceAll( '\\', '/' );
		const resolvedPath = this._resolveFilePath( cleanPath );
		let assetData = this.assets[ resolvedPath ] || this.assets[ cleanPath ];
		if ( ! assetData ) {
			const basename = cleanPath.split( '/' ).pop();
			const matches = Object.keys( this.assets ).filter( key => key.split( '/' ).pop() === basename );
			if ( matches.length !== 1 ) {
                const error = new Error( 'Missing or ambiguous USD texture: ' + cleanPath );
                error.code = 'USD_MISSING_TEXTURE';
                error.texturePath = resolvedPath;
                throw error;
            }
			assetData = this.assets[ matches[ 0 ] ];
		}
	...
	_createTextureFromData( data, textureAttrs, transformAttrs ) {
		...
		this.texturePromises.push( new Promise( ( resolve ) => {
			...
			image.onerror = function () {
				...
				if ( scope.manager ) scope.manager.itemError( 'USD texture could not be decoded' );
