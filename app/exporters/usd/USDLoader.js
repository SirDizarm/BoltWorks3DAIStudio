...
import { collectUsdMotions } from './USDMotions.js';
import { composeUsdLayers } from './USDLayerComposer.js';
	...
	parse( buffer, path = '', onLoad, onError ) {
		...
		function parseAssets( zip ) {
			...
			for ( const filename in zip ) {
				...
				if ( ext === 'png' || ext === 'jpg' || ext === 'jpeg' || ext === 'avif' || ext === 'webp' || ext === 'bmp' || ext === 'gif' ) {
		...
		const bytes = buffer?.usdFiles ? null : new Uint8Array( buffer );

		// USDZ

		if ( buffer?.usdFiles || ( bytes[ 0 ] === 0x50 && bytes[ 1 ] === 0x4B ) ) {

			const zip = buffer?.usdFiles || unzipSync( bytes );
			...
			const needsLayers=Object.values(assets).some(asset=>asset?.specsByPath&&Object.values(asset.specsByPath).some(spec=>spec.fields.subLayers||Array.isArray(spec.fields.references)&&spec.fields.references.some(ref=>typeof ref==='object')));
            const composed=needsLayers?composeUsdLayers(filename,assets):data;
            const group=composer.compose(composed,assets,{},needsLayers?'':basePath);
            group.userData.usdWarnings=[...(composed.warnings||[]),...(composer.materialWarnings||[])];
            const motions=collectUsdMotions(assets,group);
            if(motions.clips.length)group.animations=motions.clips;
            group.userData.usdWarnings.push(...motions.warnings);
            return finalize(composer,group);
